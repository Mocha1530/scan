...
(async () => {
  ...
  if (process.env.NODE_ENV === 'DEMO') {
    ...
    fastify.addHook('onRequest', async (request, reply) => {
      ...
      // check if requester ip has a session (temporary access)
      if (session) {
        //if session found, check if the session is expired
    ...
    fastify.post('/apidemo', async (request, reply) => {
      ...
      // check if the request ip has a session (temporary access)
      const session = map.get(ip);
